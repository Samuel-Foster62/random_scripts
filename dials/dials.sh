...
images=`ls -1v /CHANGE/THIS/PATH/*.nxs`
src_dir=$(pwd)
for wedge in $images
do
  echo $wedge
  ...
  echo $date
  directory=$src_dir/CHANGE/THIS/PATH/$name
  if [ -d $directory ]
  then
...
dials.find_spots /dls/science/groups/i02-1/scripts/processing/spots.phil imported.expt d_min=2.2 d_max=20.0
